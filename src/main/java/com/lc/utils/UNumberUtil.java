...
public class UNumberUtil {
    ...
    public static String toStr(Number number) {
        ...
        if (false == UObjectUtil.isValidIfNumber(number)) {
     ...
     * @deprecated 请使用{@link UCharUtil#isBlankChar(char)}
     */
    @Deprecated
     ...
     * @see Character#isWhitespace(int)
     * @see Character#isSpaceChar(int)
     * @deprecated 请使用{@link UCharUtil#isBlankChar(int)}
     */
    @Deprecated
    ...
    public static BigInteger newBigInteger(String str) {
        str = UStringUtil.trimToNull(str);
        if (null == str) {
            return null;
        }
    ...
    public static int parseInt(String number) throws NumberFormatException {
        if (UStringUtil.isBlank(number)) {
            return 0;
        }

        // 对于带小数转换为整数采取去掉小数的策略
        number = UStringUtil.subBefore(number, UCharUtil.DOT, false);
        if (UStringUtil.isEmpty(number)) {
            return 0;
        }

        if (UStringUtil.startWithIgnoreCase(number, "0x")) {
            // 0x04表示16进制数
