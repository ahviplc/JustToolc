...
public class UClassLoaderUtil {
     ...
     * 包名分界符: '.'
     */
    private static final char PACKAGE_SEPARATOR = UStringUtil.C_DOT;
    /**
     * 内部类分界符: '$'
     */
     ...
     * 2、获取{@link UClassLoaderUtil}类对应的ClassLoader
    ...
    public static ClassLoader getClassLoader() {
        ...
        if (classLoader == null) {
            classLoader = UClassLoaderUtil.class.getClassLoader();
            if (null == classLoader) {
    ...
    public static Class<?> loadPrimitiveClass(String name) {
        Class<?> result = null;
        if (UStringUtil.isNotBlank(name)) {
            name = name.trim();
            if (name.length() <= 8) {
