...
public class UReUtil {
     ...
     * 正则中需要被转义的关键字
     */
    public final static Set<Character> RE_KEYS = UCollUtil.newHashSet(new Character[]{'$', '(', ')', '*', '+', '.', '[', ']', '?', '\\', '^', '{', '}', '|'});
    ...
    public static String extractMulti(Pattern pattern, String content, String template) {
        ...
        final TreeSet<Integer> varNums = new TreeSet<>(new Comparator<Integer>() {
            ...
            public int compare(Integer o1, Integer o2) {
                return UObjectUtil.compare(o2, o1);
            }
        });
    ...
    public static String extractMultiAndDelPre(Pattern pattern, Holder<String> contentHolder, String template) {
        ...
        if (matcher.find()) {
            ...
            contentHolder.set(UStringUtil.sub(content, matcher.end(), content.length()));
            return template;
        }
        return null;
    }
    ...
    public static String delFirst(String regex, String content) {
        if (UStringUtil.hasBlank(regex, content)) {
            return content;
        }
    ...
    public static String delFirst(Pattern pattern, String content) {
        if (null == pattern || UStringUtil.isBlank(content)) {
            return content;
        }

        return pattern.matcher(content).replaceFirst(UStringUtil.EMPTY);
    }
    ...
    public static String delAll(Pattern pattern, String content) {
        ...
        return pattern.matcher(content).replaceAll(UStringUtil.EMPTY);
    }
    ...
    public static String delPre(String regex, String content) {
        ...
        Matcher matcher = pattern.matcher(content);
        if (matcher.find()) {
            return UStringUtil.sub(content, matcher.end(), content.length());
        }
        return content;
    }
    ...
    public static String replaceAll(String content, Pattern pattern, String replacementTemplate) {
        if (UStringUtil.isEmpty(content)) {
            return content;
        }
    ...
    public static String escape(String content) {
        if (UStringUtil.isBlank(content)) {
            return content;
        }
