...
public abstract class AbstractConverter<T> implements Converter<T> {
    ...
    public T convert(Object value, T defaultValue) {
        ...
        if (null == targetType && null == defaultValue) {
            throw new NullPointerException(UStringUtil.format("[type] and [defaultValue] are both null for Converter [{}], we can not know what type to convert !", this.getClass().getName()));
        }
        if (null == targetType) {
        ...
        if (null == defaultValue || targetType.isInstance(defaultValue)) {
            ...
            throw new IllegalArgumentException(UStringUtil.format("Default value [{}] is not the instance of [{}]", defaultValue, targetType));
        }
    }
    ...
    protected String convertToStr(Object value) {
        ...
        if (value instanceof String) {
            ...
            return UArrayUtil.toString(value);
        } else if (UCharUtil.isChar(value)) {
            //对于ASCII字符使用缓存加速转换，减少空间创建
            return UCharUtil.toString((char) value);
        }
    ...
    public Class<T> getTargetType() {
        return (Class<T>) UClassUtil.getTypeArgument(getClass());
    }
}
