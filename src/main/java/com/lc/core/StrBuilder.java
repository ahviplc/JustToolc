...
// ------------------------------------------------------------------------------------ Constructor start
    ...
    public StrBuilder(CharSequence... strs) {
        this(UArrayUtil.isEmpty(strs) ? DEFAULT_CAPACITY : (totalLength(strs) + DEFAULT_CAPACITY));
...
// ------------------------------------------------------------------------------------ Insert
    ...
    public StrBuilder insert(int index, char[] src, int srcPos, int length) {
        if (UArrayUtil.isEmpty(src) || srcPos > src.length || length <= 0) {
            return this;
        }
        if (index < 0) {
            index = 0;
...
// ------------------------------------------------------------------------------------ Private method start
    ...
    private void moveDataAfterIndex(int index, int length) {
        ...
        if (index < this.position) {
            ...
            Arrays.fill(this.value, this.position, index, UStringUtil.C_SPACE);
        }
        // 不位移
    }
