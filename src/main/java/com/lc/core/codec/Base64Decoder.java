...
public class Base64Decoder {
    ...
    public static String decodeStr(CharSequence source, Charset charset) {
        return UStringUtil.str(decode(source), charset);
    }
    ...
    public static byte[] decode(CharSequence source) {
        return decode(UStringUtil.bytes(source, DEFAULT_CHARSET));
    }
    ...
    public static byte[] decode(byte[] in, int pos, int length) {
        ...
        if (octetId == octet.length) {
            ...
            return (byte[]) UArrayUtil.copy(octet, new byte[octetId], octetId);
        }
    }
